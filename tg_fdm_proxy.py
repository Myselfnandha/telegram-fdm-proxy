...
_auth_key_locks: Dict[int, asyncio.Lock] = {}
dc_semaphores: Dict[int, asyncio.Semaphore] = {}

def get_dc_semaphore(dc_id: int) -> asyncio.Semaphore:
    if dc_id not in dc_semaphores:
        dc_semaphores[dc_id] = asyncio.Semaphore(3)
    return dc_semaphores[dc_id]

async def get_dc_auth_key(dc_id: int) -> Any:
    if dc_id == client.session.dc_id:
        return client.session.auth_key
    if dc_id in dc_auth_keys:
        return dc_auth_keys[dc_id]

    if dc_id not in _auth_key_locks:
        _auth_key_locks[dc_id] = asyncio.Lock()

    async with _auth_key_locks[dc_id]:
        if dc_id in dc_auth_keys:
            return dc_auth_keys[dc_id]
